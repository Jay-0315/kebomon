...
const REGIONS: RegionDef[] = [
  {
    ...
    icon: <Map size={18}/>, unlockRaidId: null, unlockDescKo: "", unlockDescJa: "",
    rewardBase: { points: 80, stones: 1, normalEgg: 1, bigEgg: 0, goldEgg: 0 },
  },
  {
    id: "forest",
    nameKo: "삼림의 비경", nameJa: "森の秘境",
...
// ── Time display ────────────────────────────────────────────────────────────
function fmtRemaining(ms: number, ko: boolean): string {
  if (ms <= 0) return ko ? "완료!" : "完了！";
  const h = Math.floor(ms / 3600000);
  const m = Math.floor((ms % 3600000) / 60000);
  const s = Math.floor((ms % 60000) / 1000);
  if (h > 0) return ko ? `${h}시간 ${m}분` : `${h}時間${m}分`;
  if (m > 0) return ko ? `${m}분 ${s}초` : `${m}分${s}秒`;
  return ko ? `${s}초` : `${s}秒`;
}

// ── Character picker modal ──────────────────────────────────────────────────
function CharPickerModal({ ownedIds, selected, maxParty, minRarity, onToggle, onClose, ko }: {
  ownedIds: number[];
  selected: number[];
  maxParty: number;
  minRarity: CharacterRarity;
    ...
    >
      ...
      >
        ...
        <p style={{ margin:"0 0 12px", fontSize:12, color:C.textDim }}>
          {ko ? `최대 ${maxParty}마리 · ${RARITY_KO[minRarity]}+ 등급만 참여 가능` : `最大${maxParty}匹 · ${RARITY_JA[minRarity]}+のみ参加可能`}
        </p>
        {eligibleChars.length === 0 && (
          <p style={{ color:C.textDim, fontSize:13, textAlign:"center", padding:"20px 0" }}>
            {ko ? "조건에 맞는 케보몬이 없습니다" : "条件に合うケボモンがいません"}
          </p>
        )}
            ...
            return (
              ...
              >
                ...
                <p style={{ margin:0, fontSize:9, color:isSel?C.green:C.textDim, fontWeight:700, textAlign:"center", lineHeight:1.3 }}>
                  {getCharName(char, ko?"ko":"ja").slice(0,6)}
                </p>
...
// ── Reward Info Popup ───────────────────────────────────────────────────────
function RewardInfoPopup({ ko, onClose }: { ko: boolean; onClose: () => void }) {
  const durationRows = [
    { hours: 2, mult: 1.0,  bonus: ko ? "기준" : "基準" },
    { hours: 4, mult: 1.6,  bonus: "+60%" },
    { hours: 6, mult: 2.1,  bonus: "+110%" },
  ];
  const diffRows = [
    { label: ko ? "낮음"  : "低",   mult: 1.0, bonus: ko ? "기준" : "基準", color: DIFF_COLOR.low },
    { label: ko ? "중간"  : "中",   mult: 1.1, bonus: "+10%",               color: DIFF_COLOR.medium },
    { label: ko ? "높음"  : "高",   mult: 1.3, bonus: "+30%",               color: DIFF_COLOR.high },
    { label: ko ? "최고"  : "最高", mult: 1.5, bonus: "+50%",               color: DIFF_COLOR.extreme },
  ];
    ...
    >
      ...
      >
        ...
        <table style={tableStyle}>
          <thead>
            <tr>
              <th style={th}>{ko ? "시간" : "時間"}</th>
              <th style={th}>{ko ? "배율" : "倍率"}</th>
              <th style={th}>{ko ? "증가량" : "増加量"}</th>
            </tr>
          </thead>
          <tbody>
            {durationRows.map(r => (
              <tr key={r.hours} style={{ borderTop: `1px solid ${C.border}` }}>
                <td style={{ ...td, color: C.gold, fontWeight: 700 }}>{r.hours}{ko ? "시간" : "h"}</td>
        ...
        <table style={tableStyle}>
          <thead>
            <tr>
              <th style={th}>{ko ? "초과 인원" : "超過人数"}</th>
              <th style={th}>{ko ? "파티 배율" : "パーティー倍率"}</th>
              <th style={th}>{ko ? "보너스" : "ボーナス"}</th>
            </tr>
          </thead>
          <tbody>
            {partyRows.map(r => (
              <tr key={r.extra} style={{ borderTop: `1px solid ${C.border}` }}>
                <td style={{ ...td, color: "#60a5fa", fontWeight: 700 }}>+{r.extra}{ko ? "마리" : "匹"}</td>
        ...
        <div style={{
          ...
          <p style={{ margin: "4px 0 0", fontSize: 10, color: C.textDim }}>
            {ko ? "* 배율은 모두 곱연산으로 적용됩니다" : "* 倍率はすべて掛け算で適用されます"}
          </p>
        </div>
      </div>
    </div>
...
export default function ExpeditionPage() {
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20, gap:20 }}>
        ...
        <div style={{ textAlign:"center" }}>
          <p style={{ margin:0, fontSize:22, fontWeight:900, color:C.gold }}>{ko ? "원정 완료!" : "遠征完了！"}</p>
          <p style={{ margin:"4px 0 0", fontSize:13, color:C.textDim }}>
            {ko ? `원정 ${totalCount}회 달성` : `遠征${totalCount}回達成`}
          </p>
        </div>
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
          <RewardBadge label={ko?"탐험 포인트":"探険P"} count={r.points} color={C.gold}/>
          <RewardBadge label={ko?"강화석":"強化石"} count={r.stones} color="#60a5fa"/>
          <RewardBadge label={ko?"일반 알":"通常卵"} count={r.normalEgg} color="#94a3b8"/>
          <RewardBadge label={ko?"고급 알":"上級卵"} count={r.bigEgg} color="#4ade80"/>
          <RewardBadge label={ko?"황금 알":"黄金卵"} count={r.goldEgg} color={C.gold}/>
        </div>
        <p style={{ fontSize:11, color:C.textDim, maxWidth:280, textAlign:"center", lineHeight:1.6 }}>
          {ko ? "* 실제 보상은 케보몬 내 강화 시스템에서 직접 적용됩니다" : "* 実際の報酬はケボモン強化システムで適用されます"}
        </p>
        <button
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT }}>
      <style>{css}</style>
      {showInfo && <RewardInfoPopup ko={ko} onClose={() => setShowInfo(false)} />}
      {showPicker && (
        <CharPickerModal
          ownedIds={ownedIds}
          selected={party}
      ...
      <div style={{ padding:"16px 16px 0", maxWidth:800, margin:"0 auto" }}>
        ...
        <p style={{ margin:0, fontSize:12, color:C.textDim }}>
          {ko ? "원정대를 꾸려 지역을 탐험하고 보상을 획득하세요" : "遠征隊を編成してエリアを探索し報酬を獲得しよう"}
        </p>
        <p style={{ margin:"4px 0 0", fontSize:11, color:C.textDim }}>
          {ko ? `누적 원정: ${totalCount}회` : `累計遠征: ${totalCount}回`}
        </p>
      </div>
              ...
              >
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ margin:0, fontSize:12, fontWeight:700, color: unlocked ? (isActive ? col : C.textBright) : C.textDim, lineHeight:1.3 }}>
                    {ko ? region.nameKo : region.nameJa}
                  </p>
                  <p style={{ margin:0, fontSize:10, color: DIFF_COLOR[region.difficulty] }}>
                    {ko ? DIFF_KO[region.difficulty] : DIFF_JA[region.difficulty]}
                    {!isDefault && !unlocked && (
                      <span style={{ color:C.textDim }}> · {ko?"잠김":"ロック"}</span>
                    )}
                  </p>
                </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div>
                <p style={{ margin:0, fontSize:15, fontWeight:900, color:selectedRegion.color }}>
                  {ko ? selectedRegion.nameKo : selectedRegion.nameJa}
                </p>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <span style={{
                    ...
                    {ko ? DIFF_KO[selectedRegion.difficulty] : DIFF_JA[selectedRegion.difficulty]}
                  </span>
                  {!isUnlocked(selectedRegion) && (
                    <span style={{ fontSize:10, color:C.textDim, display:"flex", alignItems:"center", gap:3 }}>
                      <Lock size={10}/> {ko ? selectedRegion.unlockDescKo : selectedRegion.unlockDescJa}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div style={{ padding:"10px 14px" }}>
              <p style={{ margin:"0 0 10px", fontSize:12, color:C.textDim, lineHeight:1.7 }}>
                {ko ? selectedRegion.descKo : selectedRegion.descJa}
              </p>
              {/* Requirements */}
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
                <div style={{ display:"flex", alignItems:"center", gap:4, background:"#1e2940", borderRadius:6, padding:"3px 8px" }}>
                  ...
                  <span style={{ fontSize:11, color:"#60a5fa", fontWeight:700 }}>
                    {ko ? `최소 ${selectedRegion.minParty}마리` : `最低${selectedRegion.minParty}匹`}
                  </span>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:4, background:"#1e1a30", borderRadius:6, padding:"3px 8px" }}>
                  ...
                  <span style={{ fontSize:11, color:"#a855f7", fontWeight:700 }}>
                    {(ko ? RARITY_KO : RARITY_JA)[selectedRegion.minRarity]}+
                  </span>
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <p style={{ margin:0, fontSize:12, color:"#fca5a5" }}>
                {ko
                  ...
                  : `このエリアは[${selectedRegion.unlockDescJa}]後に解放されます`
                }
              </p>
            </div>
          )}
            ...
            <>
              ...
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px" }}>
                <p style={{ margin:"0 0 10px", fontSize:12, fontWeight:700, color:C.textBright }}>
                  {ko ? "원정대 편성" : "遠征隊編成"} ({party.length}/{selectedRegion.minParty}~5)
                </p>
              ...
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px" }}>
                ...
                <p style={{ margin:"6px 0 0", fontSize:10, color:C.textDim }}>
                  {ko ? "* 파티 마리 수가 많을수록 보상이 증가합니다" : "* 隊員が多いほど報酬が増加します"}
                </p>
              </div>
              ...
              >
                {party.length < selectedRegion.minParty
                  ? ko ? `최소 ${selectedRegion.minParty}마리 필요` : `最低${selectedRegion.minParty}匹必要`
                  : ko ? "원정 출발!" : "遠征出発！"
                }
              </button>
            </>
          ...
          {expedition && !expedition.rewardClaimed && !isComplete && (
            <div style={{
              ...
              <p style={{ margin:0, fontSize:12, color:C.textDim }}>
                {ko ? "원정 진행 중입니다. 완료 후 다시 출발할 수 있습니다." : "遠征中です。完了後に再出発できます。"}
              </p>
            </div>
          )}
        </div>
